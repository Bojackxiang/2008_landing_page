var regex_email = /^[-!#$%&'*+\/0-9=?A-Z^_a-z{|}~](\.?[-!#$%&'*+\/0-9=?A-Z^_a-z`{|}~])*@[a-zA-Z0-9](-?\.?[a-zA-Z0-9])*\.[a-zA-Z](-?[a-zA-Z0-9])+$/,
  validate_email = function(a) {
    if (!a) return !1;
    if (254 < a.length) return !1;
    var b = regex_email.test(a);
    if (!b) return !1;
    var c = a.split("@");
    if (64 < c[0].length) return !1;
    var d = c[1].split(".");
    return !d.some(function(f) {
      return 63 < f.length;
    });
  },
  validate_form = function(a, b, c, d) {
    var f = !0;
    return (
      a
      ...
      !d || 0 >= d.length
        ? ((f = !1), $("#errorMsgInterest").removeClass("hidden"))
        : $("#errorMsgInterest").addClass("hidden"),
      f
    );
  },
  save_form_mock = function(a) {
    return function(b) {
      b.success(a);
    };
  };
$(document).ready(function() {
  $("#register").click(function(a) {
    ...
    var b = $('input[name="regName"]').val()
        ? $('input[name="regName"]')
            .val()
            .trim()
        : "",
      c = $('input[name="regPhone"]').val()
        ? $('input[name="regPhone"]')
            .val()
            .trim()
        : "",
      d = $('input[name="regEmail"]').val()
        ? $('input[name="regEmail"]')
            .val()
            .trim()
        : "",
    ...
    if (
      ...
      $.ajax({
        ...
        dataType: "json",
        data: JSON.stringify({ name: b, phone: c, email: d, interests: f }),
        success: function(h) {
          console.log(h),
            ...
            h && h.success
              ? (console.log("Form submitted"),
                ...
                $("#formSubmitMsg")
                  ...
                  .fadeIn(300))
              : (console.error(`Failed to submit form: ${h ? h.message : ""}`),
                $("#formSubmitMsg")
